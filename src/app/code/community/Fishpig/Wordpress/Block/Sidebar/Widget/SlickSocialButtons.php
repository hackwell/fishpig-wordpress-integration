...
class Fishpig_Wordpress_Block_Sidebar_Widget_SlickSocialButtons extends Fishpig_Wordpress_Block_Sidebar_Widget_Abstract
{
	...
	protected function _toHtml()
	{
		...
		if ($this->getText()) {
			...
			if ($this->getMethod() === 'stick') {
				$scripts[] = 'wp-content/plugins/slick-social-share-buttons/js/jquery.social.slick.1.0.js';
			}
			else {
				$scripts[] = 'wp-content/plugins/slick-social-share-buttons/js/jquery.social.float.1.3.js';
			}
	...
	protected function _getInitText()
	{
		...
		if (!$this->getTabImage()) {
			if ($this->getMethod() === 'stick') {
				$this->setTabImage(sprintf('<img src="%s/css/images/tab_%s_%s.png" alt="Share" />', $this->_getPluginDirectoryUrl(), $this->getLocation(), $this->getDirection()));
			}
			else {
				if ($width == '130') {
		...
		if ($this->getMethod() === 'stick') {
			$text = array(
				'<script type="text/javascript">try{ _gaq.trackFacebook();}catch(e){}</script>',
	...
	protected function _getPlusOneCode()
	{
		...
		$size = $parts[0];
		$count = isset($parts[1]) && !empty($parts[1]) ? ' count="true"': ' count="false"';
	...
	protected function _isHomepage()
	{
		if ($this->_getWpOption('show_on_front') == 'page') {
			...
			return in_array('wordpress_homepage', $this->getLayout()->getUpdate()->getHandles());
		}
